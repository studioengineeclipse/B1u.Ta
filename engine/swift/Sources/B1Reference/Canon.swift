...
struct CanonParser {
    ...
    private mutating func number() throws -> Int64 {
        ...
        if s[i] == "-" { i += 1 }
        let digitsStart = i
        while i < s.count, s[i].isNumber, s[i].isASCII { i += 1 }
    ...
    private mutating func hex4() throws -> UInt32 {
        ...
        var v: UInt32 = 0
        for k in 0..<4 {
            guard let d = s[i + k].hexDigitValue, s[i + k].isASCII else { throw B1Error.parse }
            v = (v << 4) | UInt32(d)
        }
        i += 4
        return v
    ...
    private mutating func string() throws -> String {
        ...
        while true {
            ...
            if c == "\\" {
                ...
                case "r": out.append("\r")
                case "t": out.append("\t")
                case "u": out.append(try unicodeEscape())
                default: throw B1Error.parse
                }
                continue
            }
            if let ascii = c.asciiValue, ascii < 0x20 { throw B1Error.parse } // raw control char
            out.append(c)
            i += 1
        }
    }
